...
//            );
    ...
    protected void drawCircleForPreviousPoint(Canvas canvas) {
        ...
        canvas.drawText(
                LoginDataFragment.usernameglobal,
                currentLinePoint.x,
                currentLinePoint.y,
                axisLabelPaint
        );
...
//                );
 ...
 //point.x = xAxisStartPoint.x + ((xAxisEndPoint.x - xAxisStartPoint.x) * (xAxisRange - (System.currentTimeMillis() - advertisingPacket.getTimestamp()))) / xAxisRange;
   //     point.y = yAxisStartPoint.y - ((yAxisStartPoint.y - yAxisEndPoint.y) * (getValue(beacon, advertisingPacket) - (float) yAxisMinimumAnimator.getAnimatedValue())) / yAxisRange;
        ...
        point.x =100+(getValue(beacon, advertisingPacket) - (float) yAxisMinimumAnimator.getAnimatedValue());// xAxisStartPoint.x + ((xAxisEndPoint.x - xAxisStartPoint.x) * (xAxisRange - (System.currentTimeMillis() - advertisingPacket.getTimestamp()))) / xAxisRange;
        point.y = yAxisStartPoint.y - ((yAxisStartPoint.y - yAxisEndPoint.y) * (getValue(beacon, advertisingPacket) - (float) yAxisMinimumAnimator.getAnimatedValue())) / yAxisRange;
        point.y=point.y+10;
       // point.x=(System.currentTimeMillis() - advertisingPacket.getTimestamp());
        //point.y  =(getValue(beacon, advertisingPacket) - (float) yAxisMinimumAnimator.getAnimatedValue());
        return point;
    }
